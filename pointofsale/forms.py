...
from pubsite.models import get_current_event
...
class BuyDrinkForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.fields['account'].choices = ( (account.pk, account.pk)
                for account in Account.objects.filter(participant__event=get_current_event()).order_by('participant__user__first_name')
                if account.get_credits_remaining() > 0
            )
        except:
